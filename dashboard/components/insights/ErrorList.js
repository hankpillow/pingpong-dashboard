...
import {pretty} from 'modules/timeformat'
	...
	render({data}) {
		...
		return (
			<div className={'error-wrapper'}>
				<hr/>
				<span>{typeof this.linkState}</span>
				foo

				<h3>Error list ({data.length})</h3>
				<a
					href={'https://cloud.docker.com/swarm/mrboots/repository/docker/mrboots/pingpong-core/general'}
					target={'_blank'}>
					from pingpong-core
				</a>

				<ul>
					{data.map(({date, exit_code}) => {
						return <li>
