...
import {groupByDay, toDate} from 'modules/data'
...
const getURL = code => 'https://curl.haxx.se/libcurl/c/libcurl-errors.html#' + code.replace(/\s+\(\d+\)$/gi, '').replace(/_/gi,'')
const prettyError = msg => msg
	.toLowerCase()
	.replace(/_/gi,' ')
	.replace(/(.+)\((\d+)\)/gmi, '$2 - $1')

const tinyProxy = date => tinyDate([date])[0]
...
class ErrorList extends Component {
	render({data}) {
		...
		const group = groupByDay(toDate(data))

		// Regular result
		return (
			<div className={'error-wrapper'}>
				<h3>Error list ({data.length})</h3>
				<a href={'https://cloud.docker.com/swarm/mrboots/repository/docker/mrboots/pingpong-core/general'}
					target={'_blank'}> from pingpong-core </a>
				<hr/>
				...
				<ul>
					{group.map(list => {
						return list.map(({date, exit_code}) => {
							return <li>
								<time datetime={date}>{tinyProxy(date)}</time>
								<a target={'_blank'}
									href={getURL(CURL_CODES[exit_code])} > {prettyError(CURL_CODES[exit_code])}
							</a>
						</li>
						})
					})}
