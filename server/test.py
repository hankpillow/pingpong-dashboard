...
from api import format_sample, format_error, parse_line
...
def test_model_sample():
    assert format_sample('') is None
    assert format_sample(SAMPLE) is None
    assert format_sample('a b c d e f g h i j k') is None
    assert format_sample(SAMPLE_FAIL.split(' ')) is None
    assert format_sample(SAMPLE.split(' ')) != None
    assert format_sample(SAMPLE.split(' '))['type'] == 'sample'

def test_model_error():
    assert format_error('') is None
    assert format_error(ERROR) is None
    assert format_error('a b c d e') is None
    assert format_error('a b c d e f') is None
    assert format_error(ERROR_FAIL.split(' ')) is None
    assert format_error(ERROR_FAIL2.split(' ')) is None
    assert format_error(ERROR.split(' ')) != None
    assert format_error(ERROR.split(' '))['type'] == 'error'

def test_parse_line():
    assert parse_line('') is None
    assert parse_line(False) is None
    assert parse_line([]) is None
